...
def wrap_lines(text: str, line_chars: int) -> list[str]:
    """Greedy word wrap honoring manual newlines. Never drops characters."""
    line_chars = max(int(line_chars), 4)
    out: list[str] = []
    for para in str(text).replace("\r", "").split("\n"):
        ...
        cur = words[0]
        for w in words[1:]:
            if len(cur) + 1 + len(w) <= line_chars:
                cur += " " + w
            else:
                out.append(cur)
                cur = w
        while len(cur) > line_chars:
            out.append(cur[:line_chars])
            cur = cur[line_chars:]
        out.append(cur)
    while out and out[-1] == "":
        out.pop()
...
class TextBlock:
    ...
    w: int
    h: int
    line_boxes: list[tuple[int, int, int, int]]   # per line: x0, y0, x1, y1 (text extents)
...
def render_block(
    ...
    out_px = max(1, int(round(line_h * (0.045 + 0.075 * es)))) if edge in ("outline", "outline_shadow") else 0
    ...
    pad = int(np.ceil(max(out_px, sh_px, glow_sigma * 2.5, line_h * 0.28))) + 1
    ...
    if sh_px:
        ...
        rgb, alpha = _over(rgb, alpha, (0.0, 0.0, 0.0), sh * (0.55 + 0.35 * es))
    if out_px:
        ring = cv2.dilate(mask, np.ones((3, 3), np.float32), iterations=out_px)
        ring = np.clip(ring - mask, 0.0, 1.0)
        rgb, alpha = _over(rgb, alpha, (0.02, 0.02, 0.02), ring * (0.75 + 0.25 * es))
    if glow_sigma > 0.0:
